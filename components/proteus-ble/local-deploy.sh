...
sudo -E /greengrass/v2/bin/greengrass-cli \
  component details --name "io.iotconnect.example.IotConnectSdkProteusBleDemo" | true

sudo -E /greengrass/v2/bin/greengrass-cli \
  deployment create \
  --remove "io.iotconnect.example.IotConnectSdkProteusBleDemo" | true
...
IOTC_CONFIG=$(cat <<EOF
{
  "io.iotconnect.example.IotConnectSdkProteusBleDemo": {
     "MERGE": {
      "IOTC_CPID": "${IOTC_CPID}",
      "IOTC_ENV": "${IOTC_ENV}"
    }
